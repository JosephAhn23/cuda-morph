...
def _check_os() -> EnvCheckResult:
    ...
    if system == "Linux":
        ...
        if any(d in distro.lower() for d in ["ubuntu", "centos", "euler", "kylin", "uos"]):
            return EnvCheckResult("OS", "ok", f"{distro} ({arch})", detail=f"Kernel: {release}")
    ...
    return EnvCheckResult("OS", "warning", f"{system} {release} — not officially supported")
...
def _check_python() -> EnvCheckResult:
    ...
    if minor < 8:
        return EnvCheckResult(
            "Python", "error", f"Python {ver} — minimum 3.8 required for torch_npu"
        )
    if minor > 12:
        return EnvCheckResult(
            "Python",
...
def _check_cann_installation() -> list[EnvCheckResult]:
    ...
    results.append(EnvCheckResult("CANN Toolkit", "ok", f"CANN {version} at {found_path}"))
...
def _check_driver_firmware() -> list[EnvCheckResult]:
    ...
    if os.path.isfile(driver_path):
        try:
            ...
            results.append(EnvCheckResult("NPU Driver", "ok", f"Driver: {driver_version}"))
        except OSError:
            results.append(
                EnvCheckResult("NPU Driver", "warning", "Driver version file unreadable")
            )
    else:
        if platform.system() == "Linux":
            ...
            results.append(
                EnvCheckResult("NPU Driver", "info", "NPU driver check skipped (non-Linux)")
            )
    ...
    if npu_smi:
        try:
            ...
            if proc.returncode == 0:
                ...
                npu_lines = [ln for ln in lines if "NPU" in ln]
                results.append(
                    EnvCheckResult(
                        "npu-smi",
                        "ok",
                        f"npu-smi available — {len(npu_lines)} device(s)",
                    )
                )
            else:
                results.append(
                    EnvCheckResult(
                        "npu-smi", "warning", f"npu-smi returned error: {proc.stderr[:200]}"
                    )
                )
        except (subprocess.TimeoutExpired, OSError):
            results.append(EnvCheckResult("npu-smi", "warning", "npu-smi timed out or failed"))
    else:
        if platform.system() == "Linux":
            results.append(
                EnvCheckResult(
...
def _check_env_vars() -> list[EnvCheckResult]:
    ...
    for var, (desc, fix) in required_vars.items():
        ...
        if val:
            ...
            results.append(
                EnvCheckResult(f"env:{var}", "warning", f"{var} not set — {desc}", fix=fix)
            )
    ...
    for var, (desc, fix) in recommended_vars.items():
        ...
        if val:
            ...
            if ascend_home in val or "Ascend" in val:
                results.append(EnvCheckResult(f"env:{var}", "ok", f"{var} includes CANN paths"))
            else:
                results.append(
                    EnvCheckResult(
                        f"env:{var}", "info", f"{var} set but may not include CANN paths", fix=fix
                    )
                )
        else:
            results.append(EnvCheckResult(f"env:{var}", "info", f"{var} not set — {desc}", fix=fix))
...
def _check_compilation_tools() -> list[EnvCheckResult]:
    ...
    if cmake:
        try:
            proc = subprocess.run([cmake, "--version"], capture_output=True, text=True, timeout=5)
...
def _check_torch_npu_installation() -> EnvCheckResult:
    ...
    try:
        ...
        return EnvCheckResult(
            "torch_npu", "error", "PyTorch not installed — prerequisite for torch_npu"
        )
    ...
    try:
        ...
        return EnvCheckResult("torch_npu", "ok", f"torch_npu {version} (backend: {backend_name})")
...
def _check_hccl_readiness() -> list[EnvCheckResult]:
    ...
    if hccl_timeout:
        results.append(EnvCheckResult("HCCL timeout", "ok", f"HCCL_CONNECT_TIMEOUT={hccl_timeout}"))
    else:
        results.append(
            EnvCheckResult(
                "HCCL timeout",
...
def _check_disk_space() -> EnvCheckResult:
    ...
    try:
        ...
        return EnvCheckResult(
            "Disk Space", "ok", f"{free_gb:.1f} GB free / {total_gb:.1f} GB total"
        )
